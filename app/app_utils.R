...
dt_all_grams1 <- dt_all_grams[dt_all_grams$'word.count' == 1, ][, ngram]
dt_all_grams1 <- unique(c(dt_all_grams1, GradyAugmented))
rm(dt_all_grams)
...
predict_word <- function (input) {
     ...
     input_tmp <- tokenize(input)
     
     status_incomplete <- 
          length(input_tmp) >
          length(
               input_tmp[!is.na(match(input_tmp, GradyAugmented[GradyAugmented != "ar"]))]
               ) |
          length(input_tmp) >
                      length(
                           input_tmp[is.na(match(input_tmp, tolower(LETTERS[c(-1,-9)])))]
                           )
     
     status_space <- str_sub(input, -1) == " "
     
     
     input <- tokenize(tail(input, 10))
     input <- input[!is.na(match(input, GradyAugmented))]
     input <- input[is.na(match(input, tolower(LETTERS[c(-1,-9)]) ))]
     ...
     matched_6gram <- dt_all_grams6[dt_all_grams6$part1 == word5, part2]
     matched_5gram <- dt_all_grams5[dt_all_grams5$part1 == word4, part2]
     matched_4gram <- dt_all_grams4[dt_all_grams4$part1 == word3, part2]
     matched_3gram <- dt_all_grams3[dt_all_grams3$part1 == word2, part2]
     matched_2gram <- dt_all_grams2[dt_all_grams2$part1 == word1, part2]
     ...
     if (status_incomplete == TRUE & status_space == FALSE) {
          prediction <- unique(
               prediction[grep(paste0("^", last(input_tmp, 1)), prediction)]
          )
     } else if (status_incomplete == TRUE & status_space == TRUE) {
          prediction <- unique(
               c(head(prediction, 3),
